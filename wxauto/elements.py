...
class WxParam:
    ...
    CHAT_TEXT_HEIGHT = 52
    CHAT_IMG_HEIGHT = 117
    DEFALUT_SAVEPATH = os.path.join(os.getcwd(), 'wxauto_savefile')
...
class WeChatBase:
    ...
    def _getmsgs_filename(self, msgitems, filename , savepic=False, savefile=False, savevoice=False):
        msgs = []
        for MsgItem in msgitems:
            if MsgItem.ControlTypeName == 'ListItemControl':
                msgs.append(self._split(MsgItem))
        ...
        for msg in msgs:
            if msg.type not in ('friend', 'self'):
                continue
            if msg.content.startswith(f"[{self._lang('图片')}]") and savepic:
                imgpath = self._download_pic_by_name(msg.control, filename=filename )
                msg.content = imgpath if imgpath else msg.content
            elif msg.content.startswith(f"[{self._lang('文件')}]") and savefile:
                filepath = self._download_file(msg.control)
                msg.content = filepath if filepath else msg.content
    ...
    def _download_pic_by_name(self, msgitem, filename):
        self._show()
...
class ChatWnd(WeChatBase):
    ...
    def GetAllMessage(self, savepic=False, savefile=False, savevoice=False):
        ...
        if not self.usedmsgid:
            self.usedmsgid = set([i[-1] for i in msgs])
        return msgs
    ...
    def GetNewMessage(self, savepic=False, savefile=False, savevoice=False):
        ...
        # using a hashmp to 
        if not self.usedmsgid: # 如果为空， 使用set 来
            # for i in self.GetAllMessage():
                # self.usedmsgid[i[-1]]= False 
            self.usedmsgid = set([i[-1] for i in self.GetAllMessage()])
            # return []
        
        MsgItems = self.C_MsgList.GetChildren() # 获取对话框的所有消息
        
        # NewMsgItems = [ i for i in MsgItems if ''.join([str(i) for i in i.GetRuntimeId()]) not in self.usedmsgid]
        # dont form the the 0 to n  
        # print(len(MsgItems), len(self.usedmsgid)) 
        all_nums = len(MsgItems)
        # new_nums = all_nums - len(self.usedmsgid)
        # # print(all_nums, new_nums)
         
        NewMsgItems = [ MsgItems[i] for i in range(all_nums) if ''.join([str(i) for i in MsgItems[i].GetRuntimeId()]) not in self.usedmsgid]

        if not NewMsgItems:
            return []
        # debug for the 
        # import pdb; pdb.set_trace()
        # 已修改的内容，可以来设这，该用户是否需要保存图片，文件，语音
        newmsgs = self._getmsgs_filename(NewMsgItems, filename= self.who, savepic= self.savepic, savefile= False , savevoice = False)
        
        
        # self.usedmsgid = [i[-1] for i in self._getmsgs(MsgItems)]
        for i in newmsgs:
            self.usedmsgid.add(i[-1]) 
        # self.usedmsgid.append([i[-1] for i in newmsgs ])
...
class WeChatImage:
    ...
    def Save(self, savepath='', timeout=10):
        ...
        if not savepath:
            ...
            # 设置保存路径的格式为对应的 savepath_时间戳.jpg 
            savepath = os.path.join(WxParam.DEFALUT_SAVEPATH,savepath, f"{savepath}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}.jpg")
